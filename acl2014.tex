...
& \textbf{Spanish}
& \textbf{Hebrew} \\ \hline
\query{drink} & \query{drink} & \query{drink} & \query{trink} & \query{biore} & \query{russian} & \query{italian} & \begin{CJK}{UTF8}{gbsn} \end{CJK} & \query{spanish} & \heb{לשתות}\\
\hline \hline
...
\begin{tabular}[c]{@{}c@{}}\query{Wein} \\ \query{Glas} \\ \query{Wasser} \\ \query{Kaffee} \\ \query{Bier}	\eat{ \\ \query{Komm} \\ \query{Trink} \\ \query{Schluck} \\ \query{Milch} \\ \query{Schnaps}}	\end{tabular} &
