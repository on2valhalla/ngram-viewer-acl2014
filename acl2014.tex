...
The previous version of the Ngram Viewer brought the power of syntactic annotations and dependencies to the Ngram Corpus\cite{lin2012syntactic}, allowing for greater specificity when searching the corpus. The features we have added above further add specificity, but also allow for generalization and grouping of similar searches, preventing the need for overly verbose queries. Below we examine uses of features in the new version of the viewer that we feel users will find the most beneficial. We show specific queries here to highlight specific usages, but wish to leave the analysis of the results to the experts.
