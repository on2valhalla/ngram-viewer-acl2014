...
\hspace*{-0.5cm}
\subfigure[]{\includegraphics[width=0.49\textwidth]{graphs/president*_all}}
\subfigure[]{\includegraphics[width=0.49\textwidth]{graphs/President*1800}}
...
\label{sec:features}
The three new search features are implemented via the two-step approach mentioned above. As shown in Figure~\ref{fig:architecture}, we maintain new lookup tables that hold intermediate results needed for efficient retrieval of the necessary expansions. Next, we describe how these intermediate results are generated and how they are used to retrieve the final results.
...
Before our additions to the Ngram Viewer, one could manually create query sets resulting in identical charts, but often would have to examine the corpus to carefully craft them. For example, Figure~\ref{fig:manual} shows manually created queries searching for specific presidents; contrarily, Figure \ref{fig:presidents}(b) is a single wildcard query over the same time range; it automatically retrieves the 10 most popular presidents and shows additional trends that would have required extra work on behalf of the user.
...
dependency link feature, and show its effect across several languages.
...
\section{Conclusions}
We have presented a new version of the Ngram Viewer with new functionalities. With the introduction of these new features, users can perform more powerful searches that show trends which were not possible to extract from earlier versions of the Viewer, as well as from the Google Books Ngram Corpus.
